...
<div class="rightcolumn">
  <div class="card">
  <?php
  echo "<h3 style='font-family:cambria;'>People who Follow you</h3><hr><br>";
  $i=1;
  $r="SELECT f_people.follower_id,f_people.followee_id,users.username,users.occupation,users.name FROM users INNER JOIN f_people ON f_people.follower_id=users.user_id";
  $rr=mysqli_query($conn,$r);
 while($rrr = mysqli_fetch_assoc($rr)){
			 if($rrr['followee_id']==$_SESSION['user_id']){
				 $ii=$rrr['follower_id'];
        		 $nama=$rrr['name'];
				 $pp=$rrr['username'];
				 $qq=$rrr['occupation'];
			 echo "<a href='details.php?var=$ii' style='font-family:Cambria; font-size:18px;'><B>$nama</B></a>
			 <font face = 'Cambria'>is a $qq <form action='followers.php' method='POST'>
        <input type=submit name=$ii class='buttonz button1' value=FOLLOW style='font-family:Cambria; font-size:15px; float:right;'>
		</form>
		<br>
